...
| Config file                  | `~/.yups/config.toml`: `version`, `yups-repo`, `yups-repo-fallback`, `inference-endpoint`, `default-model`, `advanced-model` | `version` records the installed version; initialized during `--install-yups`; a corrupt file is an explicit error instead of silent defaults.                              |
